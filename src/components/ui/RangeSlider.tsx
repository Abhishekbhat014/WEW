...
export function RangeSlider({
  ...
  const commit = useCallback(
    (next: number) => {
      ...
      const snapped = clamp(
        Math.round((next - min) / stepVal) * stepVal + min,
        min,
        max,
      );
  ...
  return (
    <div
      ...
      className={cn(
        'relative flex h-10 w-full touch-none select-none items-center overflow-hidden rounded-lg bg-muted ring-1 ring-inset ring-border',
        disabled ? 'pointer-events-none opacity-50' : active ? 'cursor-grabbing' : 'cursor-grab',
        className,
      )}
    >
      <motion.div
      ...
      <div className="pointer-events-none absolute inset-x-2 inset-y-0">
        {ticks.map((t) => {
          const tp = max === min ? 0 : ((t - min) / (max - min)) * 100;
          return (
            <span
              key={t}
